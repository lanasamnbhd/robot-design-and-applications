...
xavier = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
bias_const = tf.compat.v1.constant_initializer(0.05)
rand_unif = tf.compat.v1.keras.initializers.RandomUniform(minval=-3e-3, maxval=3e-3)
...
class PPO(object):
    ...
    def __init__(self, sess, S_DIM, A_DIM, A_LR, C_LR, A_UPDATE_STEPS, C_UPDATE_STEPS, CLIP_METHOD):
        ...
        # tf placeholders
        self.tfs = tf.compat.v1.placeholder(tf.float32, [None, self.S_DIM], 'state')
        self.tfdc_r = tf.compat.v1.placeholder(tf.float32, [None, 1], 'discounted_r')
        self.tfa = tf.compat.v1.placeholder(tf.float32, [None, self.A_DIM], 'action')
        self.tfadv = tf.compat.v1.placeholder(tf.float32, [None, 1], 'advantage')

        # critic
        with tf.compat.v1.variable_scope('critic'):
            l1 = tf.compat.v1.layers.dense(self.tfs, nhidden1, activation=None, kernel_initializer=xavier,
                                 bias_initializer=bias_const, kernel_regularizer=regularizer)
            l1 = tf.nn.relu(l1)
            l2 = tf.compat.v1.layers.dense(l1, nhidden2, activation=None, kernel_initializer=xavier, bias_initializer=bias_const,
                                 kernel_regularizer=regularizer)
            l2 = tf.nn.relu(l2)

            self.v = tf.compat.v1.layers.dense(l2, 1, activation=None, kernel_initializer=rand_unif, bias_initializer=bias_const)
            self.advantage = self.tfdc_r - self.v
            self.closs = tf.reduce_mean(input_tensor=tf.square(self.advantage))
        ...
        with tf.compat.v1.variable_scope('loss'):
            ...
            self.clipped_ratio = tf.clip_by_value(self.ratio, 1. - self.CLIP_METHOD['epsilon'],
                                                  1. + self.CLIP_METHOD['epsilon'])
            self.aloss = -tf.reduce_mean(input_tensor=tf.minimum(self.ratio * self.tfadv, self.clipped_ratio * self.tfadv))

            # entropy
            entropy = -tf.reduce_sum(
                input_tensor=self.pi.prob(self.tfa) * tf.math.log(tf.clip_by_value(self.pi.prob(self.tfa), 1e-10, 1.0)), axis=1)
    ...
    def _build_anet(self, name, trainable):
        with tf.compat.v1.variable_scope(name):
            l1 = tf.compat.v1.layers.dense(self.tfs, nhidden1, activation=None, trainable=trainable, kernel_initializer=xavier,
                                 bias_initializer=bias_const, kernel_regularizer=regularizer)
            l1 = tf.nn.relu(l1)
            l2 = tf.compat.v1.layers.dense(l1, nhidden2, activation=None, trainable=trainable, kernel_initializer=xavier,
                                 bias_initializer=bias_const, kernel_regularizer=regularizer)
            l2 = tf.nn.relu(l2)

            mu = tf.compat.v1.layers.dense(l2, self.A_DIM, activation=tf.nn.tanh, trainable=trainable,
                                 kernel_initializer=rand_unif, bias_initializer=bias_const)

            small = tf.constant(1e-6)
            mu = tf.clip_by_value(mu, -1.0 + small, 1.0 - small)

            sigma = tf.compat.v1.layers.dense(l2, self.A_DIM, activation=None, trainable=trainable, kernel_initializer=rand_unif,
                                    bias_initializer=bias_const)
